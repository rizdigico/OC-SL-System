...
const statsConfig = [
    { key: "strength",     label: "STR", icon: <Shield className="w-3.5 h-3.5" /> },
    { key: "agility",      label: "AGI", icon: <Activity className="w-3.5 h-3.5" /> },
    { key: "vitality",     label: "VIT", icon: <HeartIcon className="w-3.5 h-3.5" /> },
    { key: "intelligence", label: "INT", icon: <Brain className="w-3.5 h-3.5" /> },
    { key: "sense",        label: "PER", icon: <Eye className="w-3.5 h-3.5" /> },
];
...
export function StatusWindow({ isOpen, onClose, user, setUser }: StatusWindowProps) {
    ...
    const handleAllocate = async (statKey: string) => {
        if (user.stats.statPoints <= 0) { playError(); return; }
                    ...
                    >
                        ...
                        <div className="flex-1 space-y-1.5 text-xs">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-white font-semibold text-right max-w-[160px] truncate">{user.title || "None"}</span>
                            </div>
                            <div className="flex justify-between items-center">
                        ...
                        {statsConfig.map(({ key, label, icon }) => (
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-white text-xl font-black font-caros tabular-nums">{user.stats[key]}</span>
                                </div>
                                <button
                                    onClick={() => { playClick(); handleAllocate(key); }}
                                    disabled={user.stats.statPoints <= 0}
                                    className="w-8 h-8 flex items-center justify-center transition-all duration-200"
                                    style={user.stats.statPoints > 0 ? {
